...
def test_constant_interval_beacon_is_degenerate():
    """KNOWN LIMITATION, asserted so it can't silently regress.

    A perfect metronome beacon (constant IAT) becomes all-zero after
    mean-removal, so _compute_fft_features returns zeros and the periodicity
    gate scores it BENIGN. This documents that the gate cannot catch
    low-jitter beacons — that requires model retraining, not a threshold."""
    iats = np.full(100, 30.0, dtype=np.float32)
    feats = _fft_features(iats)
    assert np.allclose(feats, 0.0), (
        "constant-interval beacon no longer collapses to zeros — if the FFT "
        "input changed, revisit the periodicity gate assumptions"
    )
    gate = (feats[0] > 0.15) and (feats[3] < 0.85) and (feats[4] > 3.0)
    assert gate is False  # confirms the false-negative behavior is real
